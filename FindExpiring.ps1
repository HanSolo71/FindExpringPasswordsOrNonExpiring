...
$ServiceAccounts = Get-ADUser -SearchBase "DC=contoso,DC=com" -filter {Enabled -eq $True} -Properties Name,DisplayName,msDS-UserPasswordExpiryTimeComputed,PasswordNeverExpires | where {($_.PasswordExpiration = $True -or $_.
